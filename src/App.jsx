...
function AnalyticsPanel({ site, analytics }) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <StatCard icon={BarChart3} label="7d Pageviews" value={analytics?.totals?.pageviews || 0} hint="Tracked by site script" />
        <StatCard icon={Globe2} label="7d Visitors" value={analytics?.totals?.visitors || 0} hint="Unique visitor IDs" />
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/api/analytics/${selectedId}`)
  ...
  return (
    <div className="min-h-screen bg-ink text-zinc-100 lg:grid lg:grid-cols-[18rem_1fr]">
      <Sidebar sites={sites} selectedId={selectedSite?.id} setSelectedId={setSelectedId} />
      <main className="min-w-0 px-5 py-5 lg:px-7">
        <header className="mb-6 flex flex-col justify-between gap-4 border-b border-line pb-5 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">{selectedSite?.name} · {selectedSite?.url}</p>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
          ...
          {selectedSite && <AnalyticsPanel site={selectedSite} analytics={analytics} />}
        </div>
        <div className="mt-5">
